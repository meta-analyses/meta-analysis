...
for (i in 1:nrow(uoutcome)){
  # i <- 10
  acmfdata <- subset(raw_data_tp_ltpa, outcome == uoutcome$outcome[i] & pa_domain_subgroup == "LTPA" & outcome_type == "mortality") #"All-cause mortality"
  if (nrow(acmfdata) > 0){
    ...
    if (uoutcome$outcome[i] == "Coronary heart disease" || uoutcome$outcome[i] == "Cardiovascular disease" || uoutcome$outcome[i] == "Stroke")
      ...
      local_cov <- F
    cat("Outcome: ", uoutcome$outcome[i], " - ", local_cov, "\n")
...
for (i in 1:nrow(uoutcome)){
  # i <- 10
  acmfdata <- subset(raw_data_tp_ltpa, outcome == uoutcome$outcome[i] & pa_domain_subgroup == "LTPA" & outcome_type == "incidence") #"All-cause mortality"
  if (nrow(acmfdata) > 0){
    acmfdata <- getMissingVariables(acmfdata, infertotalpersons = T, kcases = T)
    # Remove when totalperson is not available for hr, and personsyears for rr/or
    acmfdata <- subset(acmfdata, !((effect_measure == "hr" & (is.na(personyrs) | personyrs == 0) ) |
