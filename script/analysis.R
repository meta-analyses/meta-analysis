...
if (total_population) {
  for (i in 1:nrow(uoutcome)) {
    ...
    for (local_outcome_type in c("Fatal", "Non-fatal", "Both")) {
      # local_outcome_type <- "Non-fatal"; i <- 3
      ...
      if (nrow(acmfdata) > 0) {
        ...
        if (nrow(acmfdata) > 0) {
          ...
          if (!is.null(dataset)) {
            ...
            if (!NO_BMI_EFFECT && local_last_knot == 0.75)
              write_csv(dataset, paste0("data/csv/", ifelse(ALT, "ALT", "main"), "-", ma_filename, ".csv"))
...
if (!NO_BMI_EFFECT && create_MA_tables){# && local_last_knot == 0.75){
  ...
  write_csv(last_quantile_df, paste0("data/csv/MA-DR/", local_last_knot,"/", "75p_diseases.csv"))
}
