...
for (i in 1:nrow(uoutcome)) {
  ...
  for(gg in unique(raw_data_gsp_ltpa$sex_subgroups)){
    for (local_outcome_type in c("Both", "Fatal", "Non-fatal")) {
      ...
      gg_title <- ifelse(gg == "1", "Male Population", "Female Population")
      
      # Print basic info re outcome, outcome type and index
      cat(
        gg_title, " - Outcome: ", uoutcome$outcom[i], " , outcome type ",
      ...
      if (local_outcome_type == "Both") {
        # Subset fatal types
        add_fdata <- subset(raw_data_gsp_ltpa, sex_subgroups == gg & outcome == uoutcome$outcome[i] & 
                              pa_domain_subgroup == local_pa_domain_subgroup & 
                              outcome_type == "Fatal")
        ...
        add_nfdata <- subset(raw_data_gsp_ltpa, sex_subgroups == gg & outcome == uoutcome$outcome[i] & 
                               pa_domain_subgroup == local_pa_domain_subgroup & 
      ...
      if (nrow(acmfdata) > 0) {
        ...
        if (nrow(acmfdata) > 0) {
          ...
          if (!is.null(dataset)) {
            ...
            # Create plot title
            plot_title <- paste0(uoutcome$outcome[i], " - ", simpleCap(dir_name), " - ", gg_title)
            plot_title <- paste0(
