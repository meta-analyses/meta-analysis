...
rm (list = ls())
# Read the data
ndata <- read.csv("data/R_variables_final (1).csv", header = T, stringsAsFactors = F, skipNul = TRUE)

data <- ndata %>% filter(outcome == "All-cause mortality")# %>% dplyr::select(ref_id...1, outcome, n_per_category, person_years_per_category, cases_per_category, effect_measure, m_met_h_wk,most_adj_effect, most_adj_lci, most_adj_uci)
...
data$n <- data$person_years_per_category

data <- data %>% rename(id = ref_number)

data <- rename(data, cases = cases_per_category)
...
all_cause <- pa
all_cause$n <- with(all_cause, ifelse(effect_measure == "hr", personyrs, n_per_category))

all_cause <- all_cause %>% rename(rr = RR, personyears = personyrs, authors = first_author)
...
acmfdata <- acmfdata %>% rename(rr = RR, Author = first_author)
rdata <- acmfdata
